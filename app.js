...
const HISTORY_PUSH_INTERVAL_MS = 60 * 60 * 1000; // 1 hora
function pushHistoryPoint(tvlWei, poolWei, addr){
  const now = Date.now();
  if(now - lastHistoryPushAt < HISTORY_PUSH_INTERVAL_MS) return;
  lastHistoryPushAt = now;
  const nowDate = new Date();
  const label = nowDate.toLocaleTimeString('es-ES', { hour:'2-digit', minute:'2-digit', second:'2-digit' });
...
document.querySelectorAll('.tab-btn').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    if(btn.dataset.view === 'whitepaper'){
      renderWhitepaperCharts();
    }
  });
});
...
function updateNetworkBanner(isWrongNetwork){
  const banner = document.getElementById('wrongNetworkBanner');
  if(banner) banner.style.display = isWrongNetwork ? 'flex' : 'none';
}

/// Comprueba la red actual de la wallet; si no es BNB Smart Chain, muestra el
/// banner de aviso y deja preparado el boton para cambiar. No bloquea el resto
/// de la conexion (el usuario puede seguir viendo la interfaz), pero las
/// llamadas al contrato fallaran hasta que cambie de red de verdad.
async function ensureCorrectNetwork(eth){
  try{
    const network = await provider.getNetwork();
    const wrong = network.chainId !== TARGET_CHAIN_ID_DEC;
    updateNetworkBanner(wrong);
  }catch(e){
    console.warn('No se pudo comprobar la red actual:', e);
  }
}

async function switchToTargetNetwork(eth){
  try{
    await eth.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: TARGET_CHAIN_ID_HEX }] });
  }catch(switchErr){
    // error 4902: la red no esta anadida todavia en la wallet -> se anade y se reintenta
    if(switchErr && switchErr.code === 4902){
      try{
        await eth.request({ method: 'wallet_addEthereumChain', params: [BSC_MAINNET_PARAMS] });
      }catch(addErr){
        toast('No se pudo añadir BNB Smart Chain automáticamente. Añádela manualmente en tu wallet.', true);
        return;
      }
    } else {
      toast('No se pudo cambiar de red automáticamente. Cámbiala manualmente en tu wallet.', true);
      return;
    }
  }
  // tras cambiar, volvemos a comprobar (chainChanged tambien lo hara, esto es por si acaso)
  if(provider) await ensureCorrectNetwork(eth);
}

document.getElementById('btnSwitchNetwork').addEventListener('click', async ()=>{
  const rawEth = await waitForEthereum();
  if(!rawEth) return;
  await switchToTargetNetwork(pickProvider(rawEth));
});
...
// ---- Galería de 12 avatares: sprites reales de personaje (pixel art) ----
// Cuerpo + cabeza + armadura/pelo compuestos a partir de Universal LPC
// Spritesheet (Liberated Pixel Cup, licencia libre CC-BY-SA 3.0 / GPL3),
// no una URL libre ni contenido subido por usuarios: sigue siendo una
// "galeria fija y segura" tal como describe el contrato, ahora con arte
// de verdad en vez de formas geometricas generadas.
const AVATAR_COUNT = 12;
function renderAvatarSVG(avatarId, size){
  size = size || 32;
  const id = ((Number(avatarId) || 0) % AVATAR_COUNT + AVATAR_COUNT) % AVATAR_COUNT;
  return `<img src="assets/avatars/avatar_${id}.webp" width="${size}" height="${size}" alt="Avatar ${id+1}" style="image-rendering:pixelated; display:block;">`;
}
...
// Umbrales/recompensas de los hitos de depósito, iguales a los del contrato
// (no expuestos como getter on-chain a proposito, para ahorrar bytecode: son
// constantes fijas e inmutables, seguras de replicar aqui).
const MILESTONES = [
  { threshold: 1, reward: 0.001 },
  { threshold: 10, reward: 0.01 },
  { threshold: 100, reward: 0.1 },
  { threshold: 1000, reward: 1 }
];
function renderMilestones(totalDepositedBNB, claimedBitmap){
  const el = document.getElementById('milestones-list');
  if(!el) return;
  el.innerHTML = MILESTONES.map((m, i) => {
    const bit = 1 << i;
    const claimed = (claimedBitmap & bit) !== 0;
    const reached = totalDepositedBNB >= m.threshold;
    const state = claimed ? 'claimed' : (reached ? 'available' : 'locked');
    const statusTxt = claimed ? '✓ Reclamado' : (reached ? '¡Disponible!' : `Faltan ${(m.threshold - totalDepositedBNB).toFixed(3)} BNB`);
    return `<div class="milestone-tile ${state}">
      <div class="ledger-lbl">Deposita ${m.threshold.toLocaleString('es-ES')} BNB</div>
      <div class="milestone-threshold">${m.reward} BNB</div>
      <div class="milestone-reward">de recompensa</div>
      <div class="milestone-status">${statusTxt}</div>
    </div>`;
  }).join('');
}
// Cuanto mas alto el prestigio, mas imponente el color/simbolo: violeta -> magenta -> naranja/rojo -> dorado intenso
const PRESTIGE_TIERS = [
  { color:'#C084FC', bg:'rgba(168,85,247,0.16)', border:'rgba(168,85,247,0.45)', glow:'rgba(192,132,252,0.55)', icon:'★' },
  { color:'#F472B6', bg:'rgba(236,72,153,0.16)', border:'rgba(236,72,153,0.45)', glow:'rgba(244,114,182,0.6)', icon:'✦' },
  { color:'#FB923C', bg:'rgba(251,146,60,0.16)', border:'rgba(251,146,60,0.45)', glow:'rgba(251,146,60,0.65)', icon:'✷' },
  { color:'#F0B90B', bg:'rgba(240,185,11,0.18)', border:'rgba(240,185,11,0.5)', glow:'rgba(240,185,11,0.7)', icon:'✸' }
];
...
function renderPrestigeBadge(level){
  ...
  const tier = PRESTIGE_TIERS[Math.min(p-1, PRESTIGE_TIERS.length-1)];
  const style = `background:${tier.bg}; color:${tier.color}; border:1.5px solid ${tier.border}; text-shadow:0 0 10px ${tier.glow};`;
  return `<span class="prestige-badge" style="${style}">${tier.icon} Prestigio ${label}</span>`;
}
...
document.getElementById('connectBtn').addEventListener('click', async ()=>{
  ...
  try{
    ...
    if(eth && eth.on && !window.__chainChangedListenerSet){
      window.__chainChangedListenerSet = true;
      eth.on('chainChanged', () => { ensureCorrectNetwork(eth); });
    }
...
async function refreshAll(){
  ...
  try{
    ...
    const grossDeposited = await contract.totalGrossDepositedAllTime();
    document.getElementById('qs-grossdeposited').textContent = fmt(grossDeposited,5)+' BNB';
    ...
    if(userAddr){
      ...
      const claimedBitmap = Number(await contract.milestonesClaimed(userAddr));
      renderMilestones(parseFloat(ethers.formatEther(u.totalDeposited)), claimedBitmap);
      ...
      try{
        ...
        document.getElementById('miCuentaPortrait').innerHTML = renderAvatarSVG(prof.avatarId, 140);
      }catch(e){
        document.getElementById('mc-username').textContent = 'Jugador sin nombre';
        document.getElementById('miCuentaPortrait').innerHTML = renderAvatarSVG(0, 140);
      }
    ...
    if(userAddr){
      ...
      if(!bp.joined){
        ...
        const equipBaseCost = window.__equipBaseCost || (window.__equipBaseCost = await contract.EQUIPMENT_BASE_COST());
        for(let slot=0; slot<4; slot++){
          ...
          const nextCost = maxed ? 0n : equipBaseCost * BigInt(level + 1);
...
function initActivityFeed(){
  ...
  function pushActivity(iconClass, html, tsMs){
  ...
  const ACTIVITY_EVENT_DEFS = [
    { icon:'deposit', filter: () => contract.filters.Deposit(), render: e => `${short(e.args.user)} depositó <b>${fmt(e.args.grossAmount,4)} BNB</b>` },
    { icon:'withdraw', filter: () => contract.filters.Withdraw(), render: e => `${short(e.args.user)} retiró <b>${fmt(e.args.netPaid,4)} BNB</b>` },
    { icon:'claim', filter: () => contract.filters.Claim(), render: e => `${short(e.args.user)} cobró <b>${fmt(e.args.amount,4)} BNB</b> en dividendos` },
    { icon:'compound', filter: () => contract.filters.Compound(), render: e => `${short(e.args.user)} reinvirtió <b>${fmt(e.args.grossAmount,4)} BNB</b>` },
    { icon:'raffle', filter: () => contract.filters.RaffleTicketBought(), render: e => `${short(e.args.user)} compró ${e.args.tickets.toString()} entrada(s) de rifa` },
    { icon:'auction', filter: () => contract.filters.AuctionBid(), render: e => `${short(e.args.bidder)} pujó <b>${fmt(e.args.amount,4)} BNB</b> en la subasta` },
    { icon:'boss', filter: () => contract.filters.BossSeasonJoined(), render: e => `${short(e.args.player)} se unió al Jefe Mundial` },
    { icon:'claim', filter: () => contract.filters.Distribution(), render: e => `Reparto ejecutado: <b>${fmt(e.args.amount,4)} BNB</b> repartidos` }
  ];

  // ---- Sondeo (polling) en vez de contract.on() ----
  // Muchos nodos RPC publicos (sobre todo en testnets) no soportan bien los
  // "filtros persistentes" (eth_newFilter/eth_getFilterChanges) que ethers usa
  // por defecto para contract.on(). El sondeo solo usa eth_getLogs con un rango
  // de bloques explicito, que es el metodo mas basico y compatible que existe.
  let lastScannedBlock = null;

  async function pollActivityFeed(){
    try{
      const latestBlock = await provider.getBlockNumber();
      const fromBlock = lastScannedBlock !== null ? lastScannedBlock + 1 : Math.max(0, latestBlock - 50);
      if(fromBlock > latestBlock){ lastScannedBlock = latestBlock; return; }

      let found = [];
      for(const def of ACTIVITY_EVENT_DEFS){
        try{
          const evs = await contract.queryFilter(def.filter(), fromBlock, latestBlock);
          evs.forEach(ev => found.push({ icon: def.icon, render: def.render, event: ev }));
        }catch(e){
          console.warn('Fallo sondeando actividad en vivo (' + def.icon + '):', e);
        }
      }
      found.sort((a,b) => a.event.blockNumber - b.event.blockNumber);
      for(const item of found){
        let tsMs = Date.now();
        try{ const block = await item.event.getBlock(); if(block) tsMs = Number(block.timestamp) * 1000; }catch(e){}
        pushActivity(item.icon, item.render(item.event), tsMs);
      }
      lastScannedBlock = latestBlock;
    }catch(e){
      console.warn('Error en el sondeo de actividad en vivo:', e);
    }
  }

  // Primera pasada: trae los ultimos 5000 bloques para que no se vea vacio al
  // cargar (backfill), luego cada 15s solo sondea lo nuevo desde ahi.
  (async function initialBackfillThenPoll(){
    try{
      const latestBlock = await provider.getBlockNumber();
      lastScannedBlock = Math.max(0, latestBlock - 5001); // el +1 de pollActivityFeed cubrira desde aqui
      await pollActivityFeed();
    }catch(e){
      console.warn('No se pudo hacer el adelanto inicial de actividad:', e);
    }
    setInterval(pollActivityFeed, 15000);
  })();
...
// ---- Historial de transacciones real (eventos on-chain de tu propia direccion) ----
// Misma tecnica que el ranking: en vez de un tope arbitrario de bloques hacia atras,
// se estima el bloque real de inicio de busqueda a partir de tu firstDepositAt (o una
// ventana de seguridad de 30 dias si todavia no has depositado nunca).
let __whitepaperChartsRendered = false;
function renderWhitepaperCharts(){
  if(__whitepaperChartsRendered) return;
  __whitepaperChartsRendered = true;

  const addrEl = document.getElementById('wp-contract-address');
  if(addrEl) addrEl.textContent = CONTRACT_ADDRESS;

  const depositCanvas = document.getElementById('wpDepositChart');
  if(depositCanvas){
    new Chart(depositCanvas.getContext('2d'), {
      type: 'doughnut',
      data: {
        labels: ['Tesoro (90%)', 'Dividend Pool (7%)', 'Creador (3%)'],
        datasets: [{ data: [90, 7, 3], backgroundColor: ['#F0B90B', '#3DD68C', '#7C8894'], borderColor: '#11161D', borderWidth: 2 }]
      },
      options: {
        responsive: true,
        plugins: {
          legend: { position: 'bottom', labels: { color: '#E8ECF1', font: { size: 11 }, padding: 12 } }
        }
      }
    });
  }

  const withdrawCanvas = document.getElementById('wpWithdrawChart');
  if(withdrawCanvas){
    new Chart(withdrawCanvas.getContext('2d'), {
      type: 'doughnut',
      data: {
        labels: ['Te pagamos (95%)', 'Dividend Pool (3%)', 'Creador (2%)'],
        datasets: [{ data: [95, 3, 2], backgroundColor: ['#3DD68C', '#F0B90B', '#7C8894'], borderColor: '#11161D', borderWidth: 2 }]
      },
      options: {
        responsive: true,
        plugins: {
  ...
  const decayCanvas = document.getElementById('wpPoolDecayChart');
  if(decayCanvas){
    const days = 100;
    const labels = [];
    const noInflow = [];
    const withInflow = [];
    let poolNoInflow = 100;
    let poolWithInflow = 100;
    const dailyRate = 0.01; // 1% diario, igual que el contrato en modo normal
    const dailyInflowExample = 1.2; // BNB/dia de ejemplo entrando por comisiones del ecosistema
    for(let d = 0; d <= days; d++){
      labels.push(d);
      noInflow.push(Number(poolNoInflow.toFixed(2)));
      withInflow.push(Number(poolWithInflow.toFixed(2)));
      poolNoInflow = poolNoInflow * (1 - dailyRate);
      poolWithInflow = poolWithInflow * (1 - dailyRate) + dailyInflowExample;
    }
    new Chart(decayCanvas.getContext('2d'), {
      type: 'line',
      data: {
        labels,
        datasets: [
          { label: 'Sin nuevos ingresos (solo decae, nunca a 0)', data: noInflow, borderColor: '#7C8894', backgroundColor: 'rgba(124,136,148,0.08)', fill: true, tension: 0.2, pointRadius: 0, borderWidth: 2 },
          { label: 'Con ingresos moderados del ecosistema', data: withInflow, borderColor: '#F0B90B', backgroundColor: 'rgba(240,185,11,0.12)', fill: true, tension: 0.2, pointRadius: 0, borderWidth: 2.5 }
        ]
      },
      options: {
        responsive: true,
        plugins: {
          legend: { position: 'bottom', labels: { color: '#E8ECF1', font: { size: 10 }, padding: 10, boxWidth: 12 } }
        },
        scales: {
          x: { title: { display: true, text: 'Días', color: '#7C8894' }, ticks: { color: '#7C8894', maxTicksLimit: 6 }, grid: { color: '#232C38' } },
          y: { title: { display: true, text: 'BNB en la pool', color: '#7C8894' }, ticks: { color: '#7C8894' }, grid: { color: '#232C38' } }
        }
      }
    });
  }
...
async function renderTransactionHistory(){
  ...
  if(!listEl || !contract || !userAddr) return;
  listEl.innerHTML = '<div class="activity-empty">Cargando tu historial…</div>';
  try{
    const BSC_AVG_BLOCK_TIME = 3;
    const SAFETY_MARGIN_BLOCKS = 3000;
    const CHUNK = 3000;
    const MAX_ATTEMPTS_PER_EVENT = 60;

    const latestBlock = await provider.getBlockNumber();
    const latestBlockData = await provider.getBlock(latestBlock);
    const nowTs = latestBlockData ? Number(latestBlockData.timestamp) : Math.floor(Date.now()/1000);

    const userInfo = await contract.users(userAddr);
    const firstDepositAt = Number(userInfo.firstDepositAt);
    let searchFloorBlock;
    if(firstDepositAt > 0){
      const secondsSince = Math.max(0, nowTs - firstDepositAt);
      searchFloorBlock = Math.max(0, latestBlock - Math.ceil(secondsSince / BSC_AVG_BLOCK_TIME) - SAFETY_MARGIN_BLOCKS);
    } else {
      searchFloorBlock = Math.max(0, latestBlock - Math.ceil(30*86400 / BSC_AVG_BLOCK_TIME));
    }

    const eventDefs = [
      { icon:'deposit', filter: contract.filters.Deposit(userAddr), render: e => `Depositaste <b>${fmt(e.args.grossAmount,4)} BNB</b>` },
      { icon:'withdraw', filter: contract.filters.Withdraw(userAddr), render: e => `Retiraste <b>${fmt(e.args.netPaid,4)} BNB</b>` },
      { icon:'claim', filter: contract.filters.Claim(userAddr), render: e => `Cobraste <b>${fmt(e.args.amount,4)} BNB</b> en dividendos` },
      { icon:'compound', filter: contract.filters.Compound(userAddr), render: e => `Reinvertiste <b>${fmt(e.args.grossAmount,4)} BNB</b>` },
      { icon:'raffle', filter: contract.filters.RaffleTicketBought(null, userAddr), render: e => `Compraste ${e.args.tickets.toString()} ticket(s) de rifa` },
      { icon:'auction', filter: contract.filters.AuctionBid(null, userAddr), render: e => `Pujaste <b>${fmt(e.args.amount,4)} BNB</b> en la subasta` },
      { icon:'boss', filter: contract.filters.BossSeasonJoined(null, userAddr), render: () => `Te uniste a una temporada del Jefe Mundial` },
      { icon:'deposit', filter: contract.filters.MilestoneClaimed(userAddr), render: e => `Reclamaste un hito de depósito: <b>${fmt(e.args.totalReward,4)} BNB</b>` }
    ];
    ...
    for(const def of eventDefs){
      ...
      let attempts = 0;
      while(toBlock >= searchFloorBlock && attempts < MAX_ATTEMPTS_PER_EVENT){
        const fromBlock = Math.max(searchFloorBlock, toBlock - CHUNK + 1);
        totalAttempts++;
        try{
          const evs = await contract.queryFilter(def.filter, fromBlock, toBlock);
          evs.forEach(ev => allEvents.push({ icon: def.icon, render: def.render, event: ev }));
        }catch(chunkErr){
          console.warn('Fallo troceando historial en rango', fromBlock, toBlock, chunkErr);
          totalFailures++;
          lastChunkErr = chunkErr;
        }
        if(fromBlock === searchFloorBlock) break;
        toBlock = fromBlock - 1;
        attempts++;
      }
    }
    ...
    if(totalAttempts > 0 && totalFailures === totalAttempts){
      // todos los tramos fallaron: esto no es "sin transacciones", es un fallo real de la consulta
      throw lastChunkErr || new Error('todos los tramos de búsqueda del historial fallaron');
    }

    if(allEvents.length === 0){
      listEl.innerHTML = `<div class="activity-empty">Todavía no tienes transacciones registradas.<br><span style="font-size:10.5px;">(buscado desde el bloque ${searchFloorBlock} hasta ${latestBlock}, ${totalAttempts} tramo(s) consultado(s))</span></div>`;
      return;
    }
    ...
    // las marcas de tiempo se piden una a una (no en paralelo) para evitar el
    // fallo de "could not coalesce error" al agrupar muchas llamadas simultaneas
    const withTime = [];
    for(const item of top){
      let ts = null;
      try{ const block = await item.event.getBlock(); ts = block ? block.timestamp : null; }catch(e){}
      withTime.push({ ...item, ts });
    }

    listEl.innerHTML = withTime.map(item => {
      const timeStr = item.ts ? new Date(item.ts*1000).toLocaleString('es-ES') : '—';
      return `<div class="activity-row">
        <div class="activity-icon ${item.icon}">●</div>
        <div class="activity-text">${item.render(item.event)}</div>
        <div class="activity-time">${timeStr}</div>
      </div>`;
    }).join('');

    document.getElementById('history-status').textContent = top.length + ' transacciones · eventos on-chain reales';
  }catch(e){
    console.error('Error cargando el historial:', e);
    const msg = (e && (e.shortMessage || e.message)) ? (e.shortMessage||e.message) : 'error desconocido';
    listEl.innerHTML = `<div class="activity-empty">No se pudo cargar tu historial ahora mismo.<br><span style="font-size:10.5px;">${msg}</span></div>`;
  }
}
...
(function initAvatarPicker(){
  ...
  for(let i=0; i<AVATAR_COUNT; i++){
    html += `<button type="button" class="avatar-swatch" data-avatar-id="${i}" aria-label="Avatar ${i+1} de ${AVATAR_COUNT}" aria-pressed="false">${renderAvatarSVG(i, 40)}</button>`;
  }
  picker.innerHTML = html;
  picker.querySelectorAll('.avatar-swatch').forEach(el=>{
    el.addEventListener('click', ()=>{
      picker.querySelectorAll('.avatar-swatch').forEach(s=>{ s.classList.remove('selected'); s.setAttribute('aria-pressed','false'); });
  ...
  if(isHidden && contract && userAddr){
    try{
      ...
      picker.querySelectorAll('.avatar-swatch').forEach(s=>{
        const isSel = Number(s.dataset.avatarId) === selectedAvatarId;
        s.classList.toggle('selected', isSel);
        s.setAttribute('aria-pressed', isSel ? 'true' : 'false');
      });
...
document.getElementById('btnClaimMilestones').addEventListener('click', ()=>{
  tx(()=>contract.claimDepositMilestones(), 'Recompensa de hito reclamada.');
});
