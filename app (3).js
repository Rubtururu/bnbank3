...
function initActivityFeed(){
  ...
  // Backfill: sin esto, el feed se veria vacio hasta que ocurriera un evento
  // nuevo despues de cargar la pagina (contract.on solo escucha hacia adelante).
  // Escaneamos una ventana reciente y acotada (no todo el historial) para dar
  // una primera impresion de actividad real sin tardar mucho.
  async function backfillRecentActivity(){
    try{
      ...
      for(const def of defs){
        ...
        }catch(e){ /* si un tramo falla aqui, simplemente se omite: esto es solo un adelanto visual */ }
      }
      if(found.length === 0) return;
      found.sort((a,b) => a.event.blockNumber - b.event.blockNumber); // mas antiguo primero, para insertarlos en orden
      const recent = found.slice(-MAX_ITEMS);
      for(const item of recent){
        let tsMs = Date.now();
        try{ const block = await item.event.getBlock(); if(block) tsMs = block.timestamp * 1000; }catch(e){}
        pushActivity(item.icon, item.render(item.event), tsMs);
      }
    }catch(e){
      console.warn('No se pudo cargar el adelanto de actividad reciente:', e);
    }
  }
  backfillRecentActivity();
...
async function renderTransactionHistory(){
  ...
  try{
    ...
    if(allEvents.length === 0){
      listEl.innerHTML = '<div class="activity-empty">Todavía no tienes transacciones registradas.</div>';
      return;
    }
